...
class GPRSkeleton(ABC):
    ...
    @abstractmethod
    def fit_gpr(self, x_train: NDArray[np.float_], y_train: NDArray[np.float_]) -> None:
    ...
    @abstractmethod
    def predict_gpr(self, x_test: NDArray[np.float_]) -> tuple[NDArray[np.float_], NDArray[np.float_]]:
        """Method to predict mean and std_dev from gpr model

        Attributes:
        ----------
            x_test: Samples from Training set.
...
class GPR:
    ...
    def predict(self, x_test: NDArray[np.float_]) -> tuple[NDArray[np.float_],NDArray[np.float_]]:
        ...
        mean, std = self.gpr_model.predict_gpr(x_test)
...
class InternalGPR(GPRSkeleton):
    def __init__(self, random_state:int = 12345) -> None:
    ...
    def fit_gpr(self, x_train: NDArray[np.float_], y_train: NDArray[np.float_]) -> None:
        ...
        with catch_warnings():
            ...
            self.gpr_model.fit(x_scaled, y_train)
    ...
    def predict_gpr(self, x_test: NDArray[np.float_]) -> tuple[NDArray[np.float_], NDArray[np.float_]]:
        ...
        with catch_warnings():
            warnings.simplefilter("ignore")
            y_pred, pred_sigma = self.gpr_model.predict(x_scaled, return_std=True)
        return y_pred, pred_sigma
